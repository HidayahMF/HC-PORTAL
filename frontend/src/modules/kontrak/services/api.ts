...
const base = import.meta.env.VITE_API_URL ?? '/api/kontrak';
...
async function request<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    if (response.status === 401 && !['/auth/login', '/auth/me'].includes(path) && location.pathname !== '/kontrak/login') location.replace('/kontrak/login');
...
export const api = {
  ...
  contractNumbers: () => request<string[]>('/contracts/numbers'),
