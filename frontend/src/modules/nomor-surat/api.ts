...
const base = import.meta.env.VITE_API_URL ?? '/api/nomor-surat';
async function request<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    if (response.status === 401 && path !== '/auth/login' && path !== '/auth/me' && window.location.pathname !== '/nomor-surat/login') window.location.replace('/nomor-surat/login');
...
export const api = {
  ...
  ,deleteAccess: (nip: string) => request<null>(`/admin/users/${encodeURIComponent(nip)}`, { method: 'DELETE' })
};
