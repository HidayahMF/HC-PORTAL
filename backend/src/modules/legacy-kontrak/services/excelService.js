...
const normalize = (value) => text(value).toLowerCase().replace(/[\s_/-]+/g, '');
const dateLike = (value) => Boolean(date(value));
const isContractNumber = (value) => /^(?:\d{1,4}|\d{1,4}\/[^/]+\/[^/]+\/[IVXLCDM]+\/\d{4})$/i.test(text(value));
const romanMonths = ['', 'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X', 'XI', 'XII'];
const contractNumber = (value, startDate) => {
    const raw = text(value);
    if (!raw || raw === '-')
        return undefined;
    if (/^\d{1,3}\/[^/]+\/[^/]+\/[IVXLCDM]+\/\d{4}$/i.test(raw))
        return raw;
    if (!/^\d{1,4}$/.test(raw) || !startDate)
        return dateLike(raw) ? undefined : raw;
...
const date = (value) => {
    ...
    if (/^\d{4}[-/]\d{1,2}[-/]\d{1,2}(?:[ T].*)?$/.test(valueText)) {
        const [y, m, d] = valueText.split(/[-/ T]/);
        return `${y}-${m.padStart(2, '0')}-${d.padStart(2, '0')}`;
    }
    const match = valueText.match(/^(\d{1,2})[-/. ]([A-Za-z]+|\d{1,2})[-/. ](\d{2,4})$/);
    if (!match)
        return '';
    const months = { jan: 1, januari: 1, january: 1, feb: 2, februari: 2, february: 2, mar: 3, maret: 3, march: 3, apr: 4, april: 4, may: 5, mei: 5, jun: 6, juni: 6, june: 6, jul: 7, juli: 7, july: 7, aug: 8, agustus: 8, august: 8, sep: 9, september: 9, oct: 10, oktober: 10, october: 10, nov: 11, november: 11, dec: 12, desember: 12, december: 12 };
...
async function importWorkbook(buffer, by) {
    const workbook = xlsx_1.default.read(buffer, { type: 'buffer', cellDates: true, raw: false });
    const sheet = workbook.Sheets[workbook.SheetNames[0]];
    const rows = xlsx_1.default.utils.sheet_to_json(sheet, { header: 1, defval: '', raw: false });
    if (!rows.length)
        throw Object.assign(new Error('File Excel kosong.'), { status: 400 });
    ...
    const headerRow = rows.slice(0, 10).findIndex(row => row.map(normalize).filter(value => headerNames.includes(value)).length >= 2);
    const hasHeader = headerRow >= 0;
    // Without a header, use A-E for the required fields and F for an optional contract number.
    const firstDataRow = rows[(hasHeader ? headerRow + 1 : 0)];
    const numberBeforeDates = !hasHeader && !date(firstDataRow?.[3]) && Boolean(date(firstDataRow?.[4])) && Boolean(date(firstDataRow?.[5]));
    const header = hasHeader ? rows[headerRow].map(normalize) : numberBeforeDates ? ['nip', 'nama', 'departemen', 'contractnumber', 'startdate', 'enddate'] : ['nip', 'nama', 'departemen', 'startdate', 'enddate', 'contractnumber'];
    const index = (names, fallback) => { const found = names.map(normalize).map(name => header.indexOf(name)).find(value => value >= 0); return found ?? fallback; };
    ...
    if (hasHeader && (startIndex < 0 || endIndex < 0 || numberIndex < 0))
        throw Object.assign(new Error('Header Excel harus memiliki kolom NIP, Nama, Departemen, Awal Kontrak, Akhir Kontrak, dan Nomor Kontrak.'), { status: 400 });
    const results = [];
    for (let i = hasHeader ? headerRow + 1 : 0; i < rows.length; i += 1) {
        const row = rows[i];
        const cellValues = row.map(text);
        const contractCell = cellValues.find(value => isContractNumber(value) && !dateLike(value));
        const rawNip = contractCell && text(row[nipIndex]) === contractCell ? '' : text(row[nipIndex]);
        const rawName = text(row[nameIndex]);
        const rawDepartment = text(row[departmentIndex]);
        const rawNumber = text(row[numberIndex]);
        if (!rawNip && !rawName && !rawDepartment && !rawNumber && row.every(value => !text(value)))
            continue;
        ...
        if (!startDate || !endDate) {
            const dateCells = row.map(date).filter(Boolean);
        ...
        const input = { nip: rawNip, employeeName: rawName || undefined, department: rawDepartment || undefined, startDate: resolvedStart, endDate: resolvedEnd, contractNumber: contractNumber(rawContractNumber, resolvedStart) };
        if (!input.contractNumber && (!input.nip || !input.startDate || !input.endDate))
            continue;
        try {
            if (!input.nip || !input.startDate || !input.endDate)
                throw new Error(`Tanggal kontrak tidak valid. Kolom D (awal): "${rawStartDate || '-'}", kolom E (akhir): "${rawEndDate || '-'}". Gunakan format DD/MM/YYYY, misalnya 16/09/2026.`);
            await (0, contractService_1.create)(input, by, { allowInactive: true, allowMissingHris: true });
            results.push({ row: i + 1, nip: input.nip, success: true, message: 'Berhasil diimport.' });
        }
        catch (error) {
            const duplicate = error instanceof Error && 'code' in error && error.code === 'DUPLICATE';
            results.push({ row: i + 1, nip: input.nip, success: false, skipped: duplicate, message: error instanceof Error ? error.message : 'Baris gagal diimport.' });
        }
    }
...
async function exportWorkbook() {
    ...
    const logoPath = path_1.default.resolve(__dirname, '../../../frontend/src/assets/logobmcbg1.png');
    ...
    result.recordset.forEach((row) => {
        ...
        output.getCell(5).numFmt = 'dd/mm/yyyy';
        output.getCell(6).numFmt = 'dd/mm/yyyy';
        if (output.number % 2 === 1)
            output.eachCell(cell => { cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF8F9FD' } }; });
    });
