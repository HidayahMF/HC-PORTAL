...
function startServer() {
  ...
  const server = app.listen(PORT, async () => {
    ...
    try {
      // 1. Migrasi skema (idempotent & versioned).
      await runMigrations();
    ...
    try {
      // 2. Job yang tersangkut status running dari proses lama ditandai gagal.
      await resetStaleJobs();
      // 3. Mulai worker antrian pengiriman.
      startWorker();
    } catch (err) {
      logger.error("Worker startup failed", { err: err?.message });
    }
    try {
      await loadAllSchedules();
      await app.get("registerSimcSchedule")?.();
      await app.get("registerSimaSchedule")?.();
      logger.info("Schedulers registered");
    } catch (err) {
      logger.error("Failed to register schedulers", { err: err?.message });
    }
  });
  ...
  const shutdown = async (signal) => {
    ...
    try {
      app.get("unregisterAllSchedules")?.();
      stopWorker();
